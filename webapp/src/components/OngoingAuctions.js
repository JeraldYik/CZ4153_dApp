...
function OngoingAuctions({ auctionDetailsList, auctFactInstance, userAccounts }) {
  ...
  useEffect(() => {
    ...
    Object.keys(auctionDetailsList).map(domain => {
      ...
      temp.push(<UserOnAuction key={domain} domain={domain} bidIncrement={values.bidIncrement/1000000000} bidEndTime={values.bidEndTime} revealEndTime={values.revealEndTime} auctFactInstance={auctFactInstance} userAccounts={userAccounts}/>)
    })
